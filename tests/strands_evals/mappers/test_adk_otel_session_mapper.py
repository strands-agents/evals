...
# ============================================================================
# Tests: CloudWatch / ADOT Format (camelCase field names)
# ============================================================================

_ADOT_SPANS_FILE = _FIXTURES_DIR / "adk_adot_spans.json"


def load_adot_spans():
    """Load ADK spans in ADOT/CloudWatch format from fixture."""
    return json.loads(_ADOT_SPANS_FILE.read_text())


class TestAdotFormat:
    """ADK spans exported via ADOT to CloudWatch use camelCase field names."""

    def setup_method(self):
        self.mapper = ADKOtelSessionMapper()
        self.spans = load_adot_spans()

    def test_adot_format_maps_to_session(self):
        """ADOT camelCase spans produce the same session structure as in-memory spans."""
        session = self.mapper.map_to_session(self.spans, SESSION_ID)
        ...
        trace = session.traces[0]
        assert trace.trace_id == "adk-adot-trace-1"
        assert len(trace.spans) == 4

        # Span types
        agent = [s for s in trace.spans if isinstance(s, AgentInvocationSpan)][0]
        tool = [s for s in trace.spans if isinstance(s, ToolExecutionSpan)][0]
        inf_spans = [s for s in trace.spans if isinstance(s, InferenceSpan)]
        assert len(inf_spans) == 2
        ...
        assert "math assistant" in agent.system_prompt
        assert agent.available_tools[0].name == "calculator"
        assert agent.span_info.span_id == "invoke-agent-1"
        assert agent.span_info.parent_span_id == "invocation-1"
        assert agent.span_info.start_time.year == 2026

        # Tool execution
        assert tool.tool_call.name == "calculator"
        assert tool.tool_call.arguments == {"expression": "15 * 37"}
        assert tool.tool_call.tool_call_id == "tool-call-1"
        assert "555" in tool.tool_result.content

        # Inference
        assert inf_spans[0].metadata["model"] == "bedrock/us.anthropic.claude-sonnet-4-6"
        assert inf_spans[0].metadata["input_tokens"] == 667
