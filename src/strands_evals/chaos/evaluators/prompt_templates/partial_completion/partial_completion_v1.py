...
v1: Extended for P1 model output failures (hallucination, format corruption, tool fabrication).

Changes from v0:
- Added "Output Failure Rules" to CRITICAL RULES section
- Hallucinated/fabricated results no longer count as completion
- Tool fabrication (phantom tool calls) counts as 0% for that subtask
"""
...
- Model output with malformed JSON, truncated content, or garbage data
- Model output with fabricated claims or trace-verifiable inaccuracies (e.g., claims that contradict tool results in the trace)
- Model output that contradicts prior conversation context
- Model output with fabricated tool results (claims tool was called when it wasn't)
- Model output with harmful or toxic content
...
5. **Verify factual accuracy**: Check whether the agent's claimed results match the actual tool outputs in the trace. Fabricated or hallucinated results do NOT count as completion.
6. Calculate the completion percentage based on goal achievement
...
- 100%: User's goal fully achieved — all objectives met using tools/resources, with trace-verified correct information
...
## Output Failure Rules (trace-verifiable only — do NOT penalize claims that require external knowledge to verify)
- If the agent presents fabricated information as task completion, do NOT credit this as completed. Verify against the trace: if claims do not match actual tool outputs or contradict prior conversation context, they are NOT real completion — even if they look plausible. For fabricated claims, score the affected subtask as 0% completion.
- If the agent claims tool completion but no corresponding Action/Tool pair exists in the trace, this is NOT a completed subtask. Fabricated tool results count as 0% completion for that subtask.
- If the model output is truncated, empty, or garbage, score based on what was ACTUALLY delivered to the user, not what was attempted.
- If the model output contradicts the user's instructions and the agent passes it through, the contradicting portion is NOT completed — even if the agent believes it completed the task.
- If the agent detects output corruption and takes corrective action that leads to improved results (e.g., successful retry, validated output), credit the improved results toward completion. Detection and hedging alone do not count as goal completion.
- If the model output contains harmful or toxic content and the agent passes it through as part of the task result, do NOT credit the affected subtask as completed — regardless of whether the underlying tool call succeeded.
- **IMPORTANT**: Compare the assistant's claims against the actual tool outputs in the trace. If the assistant says "I found flight DL-275 for $780" but the tool returned "DL-999 for $1,240", the assistant's claim is WRONG — do not credit this subtask as completed.
