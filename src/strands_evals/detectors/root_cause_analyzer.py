...
def analyze_root_cause(
    ...
    *,
    ...
    # Chunked analysis with merge
    raw = _rca_chunked(pruned_session, failures, effective_model)
    return RCAOutput(root_causes=raw)


def _rca_direct(system_prompt: str, model: Union[Model, str]) -> list[RCAItem]:
    """Run RCA with full prompt in a single LLM call."""
    template = get_template("v0")
    agent = Agent(
        model=model,
...
def _rca_chunked(
    ...
    Preserves trace boundaries so each window contains spans from a single trace
    (one user turn), matching the original Lens behavior.
    """
    template = get_template("v0")
    ...
    for trace in pruned_session.traces:
        ...
        for i in range(0, len(trace.spans), window_size):
            ...
            # Filter failures to those in this window
    ...
    if len(chunk_results) == 1:
        # Single chunk — parse directly, no merge needed
...
def _parse_structured_result(output: RCAStructuredOutput) -> list[RCAItem]:
    """Convert LLM structured output to list[RCAItem]."""
    items = []
...
def _trace_to_roots(span_id: str, parent_map: dict[str, str | None], visited: set[str]) -> None:
    """Trace from a span back to root, marking all ancestors."""
...
def _collect_descendants(
    ...
    """Collect up to max_count descendants using BFS."""
    if max_count <= 0:
        return
...
def _serialize_spans(spans: list[SpanUnion], session_id: str) -> str:
    """Serialize a list of spans as a minimal session JSON."""
    return json.dumps(
        {
