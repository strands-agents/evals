...
class SummaryOutput(BaseModel):
    """Structured summary (when output_format='structured').

    NOTE: use `dict` fields for simplicity. Follow-up work should replace
    these with proper nested Pydantic models (PrimaryGoal, ToolUsage, etc.)
    matching the Lens StructuredExecutionSummary schema for better IDE support
    and runtime validation.
    """

    primary_goal: dict
    key_constraints: list[dict] = Field(default_factory=list)
    approach_taken: dict
    tools_used: list[dict] = Field(default_factory=list)
    key_discoveries: list[dict] = Field(default_factory=list)
    final_outcome: dict
