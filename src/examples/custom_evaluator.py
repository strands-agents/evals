...
async def async_simple_example():
    """
    Demonstrates creating and using a custom evaluator with ROUGE score metrics asynchronously.
    ...
    dataset = Dataset[str, str](cases = [test_case1, test_case2],
                                evaluator = TestSimilarityEvaluator())
    ### Step 3.5: Save the dataset ###
    dataset.to_file("custom_async_evaluator_dataset", "json")
...
if __name__ == "__main__":
    ...
    print(report, end_time - start_time) #  0:00:08.355666
    report.to_file("custom_report", "json")
    # report.display(include_input=False)

    start_time = datetime.datetime.now()
    report = asyncio.run(async_simple_example())
    end_time = datetime.datetime.now()
    print(report, end_time - start_time) # 0:00:05.974051
    report.to_file("custom_async_report", "json")
    report.display()