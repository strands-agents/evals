...
FAKE_DATABASE = {
    ...
    "orders": {
        "order789": {"customer_id": "user123", "product": "pro_plan", "status": "shipped", "date": "2024-01-20"},
        "order101": {"customer_id": "user456", "product": "basic_plan", "status": "processing", "date": "2024-01-22"},
    },
    "tickets": [],
}
...
async def async_agents_as_tools_example():
    """
    ...
    trajectory_evaluator = TrajectoryEvaluator(
        rubric="Does the orchestrator route customer queries to the appropriate support specialist? Score 1.0 if the routing is correct (technical→technical_support, billing→billing_support, product→product_info, orders→returns_exchanges), 0.5 if partially correct, 0.0 if incorrect."
    )
    interaction_evaluator = InteractionsEvaluator(
        rubric="Does each specialist agent use the appropriate tools and provide complete, helpful responses? Score 1.0 if tools are used correctly and response is comprehensive, 0.5 if partially adequate, 0.0 if inadequate."
    )
